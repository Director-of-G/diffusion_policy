"""
    This script is modified from real_pusht_image_dataset.py
    to support real leap hand data with image.
"""
...
class RealLeapImageDataset(BaseImageDataset):
    def __init__(self,
            shape_meta: dict,
            dataset_path: str,
            horizon=1,
...
def _get_replay_buffer(dataset_path, shape_meta, store):
    ...
    for key, attr in obs_shape_meta.items():
        ...
        if type == 'rgb':
            ...
            if 'joint' in key:
                assert tuple(shape) in [(16,)]
            if "pose" in key:
                assert tuple(shape) in [(7,)]
    
    action_shape = tuple(shape_meta['action']['shape'])
    assert action_shape in [(16,)]
